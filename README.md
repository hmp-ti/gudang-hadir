...
Aplikasi ini dibangun menggunakan _stack_ teknologi modern:
...
4.  **Setup Gemini AI**
    - Kunjungi [Google AI Studio](https://aistudio.google.com/) dan login dengan akun Google Anda.
    - Klik **Get API key** di pojok kiri atas, lalu pilih **Create API key**.
    - Salin API key yang muncul.
    - Buat file baru bernama `.env` di root folder project (sejajar dengan pubspec.yaml).
    - Isi file `.env` dengan format berikut:
      ```env
      GEMINI_API_KEY=Tempel_API_Key_Anda_Disini
      ```
    - Simpan file. Aplikasi akan otomatis menggunakan key ini saat dijalankan.
...
_Made for educational purposes and internal management solutions._
